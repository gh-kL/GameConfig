 ...
 */
export class GenETCSModule {
    // ------------------began 单例 ------------------
    private static _instance: GenETCSModule;

    public static get Instance() {
        if (this._instance == null) {
            this._instance = new GenETCSModule();
        }
    ...
    public gen(exportID: number): boolean {
        ...
        IOUtils.deleteFolderFileByCondition(this._export.export_com_url, (fullPath) => {
            // 不删除 meta 文件
            if (path.extname(fullPath) == ".meta") {
                return;
            }
        ...
        IOUtils.makeDir(this._export.export_com_url);

        // 拷贝固定代码
        IOUtils.copy(`templates/${this._export.template_name || this._export.id}/scripts/`, this._export.export_com_url);
    ...
    private genEnum(): boolean {
        ...
        for (let idx = 0; idx < enumNames.length; idx++) {
            ...
            IOUtils.writeTextFile(path.join(this._export.export_com_url, enumName + "." + this._export.script_suffix), writeContent, LineBreak.CRLF, "导出枚举类脚本成功！-> {0}");
        }
        ...
        for (let n = 0; n < this._configNames.length; n++) {
            ...
            if (cfg.fixed_keys) {
                ...
                if (!parentRemark) {
                    ...
                    cwField.add(2, `public ${uniqueKeyType} UniqueKey { set; get; }`);
                    ...
                    if (!configRemark.isSingleMainKey) {
                        ...
                        for (let mksIdx = 0; mksIdx < mainKeyNames.length; mksIdx++) {
                            ...
                            cwField.add(2, `public ${mainKeyType} ${upperMainKeyFieldName} { set; get; }`);
                ...
                IOUtils.writeTextFile(path.join(this._export.export_com_url, configName + DataModel.Instance.config.export_item_suffix + "." + this._export.script_suffix), writeContent, LineBreak.CRLF, `导出配置Item(${configRemark.sheetType})脚本成功！-> {0}`);
                ...
                for (const fixed_key in cfg) {
                    ...
                    cwField.add(2, `public ${valType} ${upperCamelFixedKey} { set; get; }`, false);
                ...
                IOUtils.writeTextFile(path.join(this._export.export_com_url, configName + "." + this._export.script_suffix), writeContent, LineBreak.CRLF, `导出配置(${configRemark.sheetType})脚本成功！-> {0}`);
            }
        }
    ...
    private genMgr(): boolean {
        let configManagerTemplate = CommonUtils.getTemplate(this._export, "ConfigManager.txt");
        let configMgrComponentTemplate = CommonUtils.getTemplate(this._export, "ConfigComponent.txt");
        let cwField = new CodeWriter();
        ...
        for (let idx = 0; idx < this._configNames.length; idx++) {
            ...
            if (fixed_keys) {
                ...
                // if (!parentLayer) {
                cwField.add(2, `public BaseConfig<${uniqueKeyType}, ${itemClassName}> ${configName} { set; get; }`, false);
                // }
                ...
                for (let idx2 = 0; idx2 < fixed_keys.length; idx2++) {
                    ...
                    if (field?.enum) {  // 处理枚举
                        ...
                        if (field.linkIsArray) {   // 处理表连接（数组形式）
                            ...
                            constructorArgsContent += `self.${linkConfigName}.Get(${StrUtils.format(DataModel.Instance.getParseFuncNameByType(CSTypeEnum.Int, this._codeLang), `lines[n + ${idx2 + idxOffset}]`)})`;
                        }
                    } else {    // 常规
                ...
                if (!parentLayer) {
                    cwParse.add(3, `self.${configName} = new BaseConfig<${uniqueKeyType}, ${itemClassName}>("${configName}", ${dictVarName});`, false);
                } else {
                    cwParse.add(3, `self.${configName} = new BaseConfig<${uniqueKeyType}, ${itemClassName}>("${configName}", ${dictVarName}_self);`, false);
                }
                ...
                if (!configRemark.isSingleMainKey) {
                    ...
                    cwField.add(2, `public ${collectionTypeStr} ${varName} { set; get; }`);

                    cwParse.newLine();

                    cwParse.add(3, `self.${varName} = new ${collectionTypeStr}();`);
                    cwParse.add(3, `foreach (var keyValuePair in self.${upperCamelConfigName}.Data)`);
                    cwParse.add(3, `{`);
                    cwParse.add(4, `var item = keyValuePair.Value;`);

                    for (let mksIdx = 1; mksIdx < mainKeyNames.length; mksIdx++) {
                        cwParse.add(4, `if (!self.${varName}`, false);
                        ...
                        cwParse.add(5, `self.${varName}`, false);
                        end = mksIdx;
                        for (let w = 0; w < end; w++) {
                            let keyNum = w + 1;
                            if (w != end - 1) {
                    ...
                    cwParse.add(4, `self.${varName}`, false);
                ...
                cwField.add(2, `public ${configName} ${configName} { set; get; }`, false);
                ...
                cwParse.add(3, `self.${configName} = new ${configName}("${configName}", ${constructorArgsContent});`, false);
            }
        ...
        let writeField = StrUtils.format(configMgrComponentTemplate,
            cwField.content,
            // cwParse.content,
        );
        ...
        IOUtils.writeTextFile(path.join(this._export.export_com_url, `${this._export.export_config_com_name}.${this._export.script_suffix}`), writeField, LineBreak.CRLF, `成功导出配置组件脚本（${this._export.export_config_manager_name}）-> {0}`);
